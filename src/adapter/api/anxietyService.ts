...
import anxietyConfig, { mentalHealthEndpoint } from '../../infrastructure/keraltyApi/repositories/anxietyConfig';


export const anxietyApi = createApi({
	baseQuery: axiosBaseQuery({
		baseUrl: `${appConfig.API_URL}/${mentalHealthEndpoint}`,
	...
	reducerPath: 'anxietyApi',
	endpoints: (build) => ({
		getYourAnxiety: build.mutation<any, object>({
			query: (obj) => anxietyConfig.getMyAnxiety(obj)
		}),
		getAnxietyParams: build.mutation<any, string>({
			query: (type) => anxietyConfig.getMyAnxietyParams(type)
		}),
		sendAnxietyParams: build.mutation<any, object>({
			query: (obj) => anxietyConfig.setDataParams(obj) 
		})	
	})
});
const anxietyService = anxietyApi;
