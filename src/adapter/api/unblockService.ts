...
import { GetIdBelongState, GetPreviousProc, SendCodeMsj, reSendCode, SendPass, SendCode, getTimeCodeExpired, ChangePasswordDTO, PersonalInfFP, ValidateRequestFP, ValidateStates } from 'infrastructure/keraltyApi/models/auth';
//


export const unblockApi = createApi({	
	...
	reducerPath: 'unblockApi',
	endpoints: (build) => ({
		GetBelongIdState: build.mutation<any, GetIdBelongState>({
			query: (data) => AuthConfig.GetIdBelong(data)
		}),	
		GetHasPreviousProc: build.mutation<any, GetPreviousProc>({
			query: (data) => AuthConfig.GetPreviousProccess(data)
		}),	
		sendRecoverAccount: build.mutation<any, SendCodeMsj>({
			query: (data) => AuthConfig.sendCodeValidate(data)
		}),	
		reSendCode: build.mutation<any, reSendCode>({
			query: (data) => AuthConfig.reSendCodeUser(data)
		}),
		sendCode: build.mutation<any, SendCode>({
			query: (data) => AuthConfig.sendCodeUser(data)
		}),	
		sendPass: build.mutation<any, SendPass>({
			query: (data) => AuthConfig.sendValuePass(data)
		}),	
		codeTimeExpired: build.mutation<any, getTimeCodeExpired>({
			query: (data) => AuthConfig.getCodeTimeExpired(data)
		}),	
...
const UnblockService = unblockApi;
