...
import SSOConfig from 'infrastructure/keraltyApi/repositories/ssoConfig';
// INFRASTRUCTURE
...
export const SSOApi = createApi({
	baseQuery: axiosBaseQuery({
		baseUrl: `${appConfig.BACK_URL}/auth`,
		prepareHeaders: (headers: any, { getState }) => {
			let state: any = getState();
			if(state.user.tokenFB) {
				headers.set('SSOToken', state.user.tokenFB);
			}
	...
	reducerPath: 'SSOApi',
	endpoints: (build) => ({
		ssoAuth: build.mutation<undefined, any>({
			query: (data) => SSOConfig.ssoAuth(data)
		}),
	})
});
const SSOService = SSOApi;
