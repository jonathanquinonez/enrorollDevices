...
import { LoadUserBySession, LoadUserBySessionResponseDTO } from 'infrastructure/keraltyApi/models/auth';
...
export const usersApi = createApi({
	...
	endpoints: (build) => ({
		...
		notificationApp: build.mutation({
			query: () => UsersConfig.notificationApp()
		}),
		...
		notificationViewAll: build.mutation({
			query: () => UsersConfig.notificationViewAll()
		}),
		remindLater: build.mutation<any, {id: string}>({
			query: (data) => UsersConfig.remindLater(data.id)
		}),
		deleteNotification: build.mutation<any, {id: string}>({
			query: (data) => UsersConfig.deleteNotification(data.id)
		}),
	})
});
