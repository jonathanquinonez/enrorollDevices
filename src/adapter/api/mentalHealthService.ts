...
import mentalHealthConfig, { mentalHealthEndpoint } from '../../infrastructure/keraltyApi/repositories/mentalHealthConfig';
// INFRASTRUCTURE
...
export const mentalHealthApi = createApi({
	...
	endpoints: (build) => ({
		getAllBaseInfo: build.mutation<any, void>({
			query: () => mentalHealthConfig.getAllBaseInfo()
		}),
		getAllDirectoryInfo: build.mutation<any, void>({
			query: () => mentalHealthConfig.getAllDirectoryInfo()
		}),
		getFile: build.mutation<any, { uuid: string }>({
			query: (data) => mentalHealthConfig.getFile(data)
		}),
	})
});
const MentalHealthService = mentalHealthApi;
