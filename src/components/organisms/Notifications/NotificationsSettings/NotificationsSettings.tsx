...
const NotificationsSettings: React.FC<Props> = (props) => {
	...
	const [change_password, setchange_password] = useState<UserNotifications>(
		initialDataNf('change_password'),
	);
	const [block_account, setblock_account] = useState<UserNotifications>(
		initialDataNf('block_account'),
	);
	const [log_in, setlog_in] = useState<UserNotifications>(initialDataNf('log_in'));
	const [biometrical_login, setbiometrical_login] = useState<UserNotifications>(
		initialDataNf('biometrical_login'),
	);
	const [annual_wellness_visit, setannual_wellness_visit] = useState<UserNotifications>(
		initialDataNf('annual_wellness_visit'),
	);
	const [update_insurance_information, setupdate_insurance_information] =
		useState<UserNotifications>(initialDataNf('update_insurance_information'));
	const [new_referral, setnew_referral] = useState<UserNotifications>(
		initialDataNf('new_referral'),
	);
	const [edit_profile, setedit_profile] = useState<UserNotifications>(
		initialDataNf('edit_profile'),
	);
	...
	const securityChange = (
		state: Dispatch<SetStateAction<UserNotifications>>,
	...
	return (
		<View style={{}}>
			...
			<ScrollView style={{ marginTop: 30, zIndex: 0, height: '100%', display: 'flex' }}>
				...
				{annual_wellness_visit && (
					<CardNotificationsSettings
						onChangeData={setannual_wellness_visit}
						key={'annual_wellness_visit'}
						data={annual_wellness_visit}
					/>
				)}
				...
				{change_password && (
					<CardNotificationsSettings
						onChangeData={(data, v) =>
							v ? setchange_password(data) : securityChange(setchange_password, data)
						}
						key={'change_password'}
				...
				{block_account && (
					<CardNotificationsSettings
						onChangeData={(data, v) =>
							v ? setblock_account(data) : securityChange(setblock_account, data)
						}
						key={'block_account'}
						data={block_account}
					/>
				...
				{log_in && (
					<CardNotificationsSettings
						onChangeData={(data, v) =>
							v ? setlog_in(data) : securityChange(setlog_in, data)
						}
						key={'log_in'}
						data={log_in}
					/>
				...
				{biometrical_login && (
					<CardNotificationsSettings
						onChangeData={(data, v) =>
							v
								? setbiometrical_login(data)
								: securityChange(setbiometrical_login, data)
						}
						key={'biometrical_login'}
						data={biometrical_login}
					/>
				)}
				{update_insurance_information && (
					<CardNotificationsSettings
						onChangeData={(data, v) =>
							v
								? setupdate_insurance_information(data)
								: securityChange(setupdate_insurance_information, data)
						}
						key={'update_insurance_information'}
						data={update_insurance_information}
					/>
				)}
				{edit_profile && (
					<CardNotificationsSettings
						onChangeData={(data, v) =>
							v ? setedit_profile(data) : securityChange(setedit_profile, data)
						}
						key={'edit_profile'}
						data={edit_profile}
					/>
