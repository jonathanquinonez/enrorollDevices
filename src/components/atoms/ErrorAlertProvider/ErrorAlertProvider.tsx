...
// Types, Styles
import {
	AlertErrorContextProps,
	ErrorAlertProviderProps as Props,
} from './ErrorAlertProvider.types';
...
const ErrorAlertProvider: React.FC<any> = (props) => {
	...
	const [remindLaterUndo] = UsersService.useRemindLaterUndoMutation();
	...
	// When error is equal to "null", system will close the notification
	const setAlertErrorMessage = (
		error: string | null,
		type?: 'error' | 'warning' | 'success' | 'successNotification',
	) => {
		if(!error) {
	...
	const post_undo = useCallback(async () => {
		try {
			if (tempDeleteNotification?.type == 'delete') {
				await createNotificationApp({
					...tempDeleteNotification?.data,
					viewed: true,
					state: locationSelected,
				}).unwrap();
			} else {
				await remindLaterUndo({
					id: tempDeleteNotification?.data.uuid ?? '',
					state: locationSelected || 'FL',
				}).unwrap();
			}
			dispatch(
				userActions.setTempDeleteNotification({
					type: 'delete',
					data: tempDeleteNotification?.data,
					status: true,
				}),
			);
		} catch (error) {
	...
	}, [createNotificationApp, tempDeleteNotification, remindLaterUndo]);
				...
				>
					<View style={{ padding: 6 }}>
						<IconButton
							...
							icon={
								typeAlert == 'success' || typeAlert == 'successNotification' ? (
									<CheckOk />
								) : typeAlert == 'warning' ? (
									<Circle />
								) : (
					...
					<View style={styles.containerText}>
						...
						<Button
							title={
								typeAlert == 'successNotification'
									? t('notifications.btnUndo')
									: t('common.close')
							}
							accessibilityRole="button"
							accesibilityLabel={t('accessibility.linkCancel2')}
							onPress={() => {
								if (typeAlert == 'successNotification') post_undo();
