...
export default function useVersion() {
	...
	const requestToken = async () => {
		return await axios.post(`${appconfig.API_URL}/core/opt-ins/authenticate`, body);
	
	};

	const requestVersion = async (token: string) => {
		return axios({
			method: 'get',
			url: `${appconfig.API_URL}/core/version/${Platform.OS}`,
			headers: {
